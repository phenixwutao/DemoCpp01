...
/*------------------- Implementing Parallel Map With Threads -------------------*/
...
unsigned int get_no_of_threads()
{
...
void parallel_map1(Iter begin, Iter end, F f)
{
...
void Ch08_DemoImplementingParallelMapWithThreads()
{
  ...
  for (auto const size : sizes)
  {
    ...
    auto tpm = perf_timer<>::duration([&] {
      parallel_map1(std::begin(v2), std::end(v2), [](int const i) {return i + i; }); });
...
/*----------------------------- Demo Implementing Parallel Map With Tasks -----------------------------*/
template <typename Time = std::chrono::microseconds,
          typename Clock = std::chrono::high_resolution_clock>
struct perf_timer2
{
